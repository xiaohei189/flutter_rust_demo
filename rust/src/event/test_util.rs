...
use crate::event::events::connection::ConnectionListener;
use crate::event::events::conversation::ConversationListener;
use crate::event::events::friend::FriendListener;
use crate::event::events::group::GroupListener;
use crate::event::events::message::MessageListener;
use crate::event::events::user::UserListener;
use std::sync::Arc;

pub(crate) fn noop_connection_listener() -> Arc<dyn ConnectionListener> { Arc::new(NoopConnectionListener) }
pub(crate) fn noop_conversation_listener() -> Arc<dyn ConversationListener> { Arc::new(NoopConversationListener) }
pub(crate) fn noop_friend_listener() -> Arc<dyn FriendListener> { Arc::new(NoopFriendListener) }
pub(crate) fn noop_group_listener() -> Arc<dyn GroupListener> { Arc::new(NoopGroupListener) }
pub(crate) fn noop_user_listener() -> Arc<dyn UserListener> { Arc::new(NoopUserListener) }
pub(crate) fn noop_message_listener() -> Arc<dyn MessageListener> { Arc::new(NoopMessageListener) }
