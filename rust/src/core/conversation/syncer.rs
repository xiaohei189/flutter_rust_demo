...
impl ConversationSyncer {
    ...
    pub async fn sync_incremental(&self) -> Result<Vec<LocalConversation>> {
        ...
        if resp.full {
            ...
            let conversations = self.sync_full().await?;
            // 对齐 Go SDK `VersionSynchronizer.IncrementalSync`：full 路径同样持久化版本号，
            // 否则下次增量请求仍无版本号，服务端会持续返回 full=true 导致每次全量同步
            if let Err(e) = self
                .repositories
                .sync_version_repo
    ...
    /// 此处补齐该逻辑。
    pub async fn sync_conversation_hash_read_seqs(&self, max_seq_recorder: &crate::core::message::MaxSeqRecorder) -> Result<()> {
...
mod tests {
    ...
    use crate::core::message::MaxSeqRecorder;
    use crate::domain::model::UserId;
    ...
    async fn test_conversation_syncer_creation() {
        ...
        let syncer = ConversationSyncer::new(http_client, repositories, UserId::new("test_user"), crate::core::event::test_util::noop_conversation_listener());
