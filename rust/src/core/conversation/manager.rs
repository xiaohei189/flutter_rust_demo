...
impl ConversationManager {
    ...
    pub fn dao(&self) -> Arc<crate::infra::database::ConversationDao> {
        self.stores.conversation_dao.clone()
    }

    pub async fn get_all_conversations(&self) -> Result<Vec<LocalConversation>> {
        let mut local_convs = self.stores.conversation_dao.get_all().await?;

        // 回填空的 latest_msg（从消息数据库获取最新消息）
        for conv in &mut local_convs {
            if conv.latest_msg.is_empty() {
                if let Ok(messages) = self.stores.message_dao.get_latest(&conv.conversation_id, 1).await {
                    if let Some(latest) = messages.first() {
                        conv.latest_msg = latest.content.clone();
                        conv.latest_msg_send_time = latest.send_time;
                        let _ = self.stores.conversation_dao.update_latest_msg(
    ...
    pub async fn get_conversation(&self, conversation_id: &str) -> Result<Option<LocalConversation>> {
        self.stores.conversation_dao.get_by_id(conversation_id).await
    }
    ...
    pub async fn delete_conversation(&self, conversation_id: &str) -> Result<()> {
        self.stores.conversation_dao.delete(conversation_id).await?;
    ...
    pub async fn set_pinned(&self, conversation_id: &str, is_pinned: bool) -> Result<()> {
        self.stores.conversation_dao.set_pinned(conversation_id, is_pinned).await?;
        info!("会话 {} 置顶状态设置为: {}", conversation_id, is_pinned);
        Ok(())
    }

    pub async fn set_private_chat(&self, conversation_id: &str, is_private: bool) -> Result<()> {
        self.stores.conversation_dao.set_private_chat(conversation_id, is_private).await?;
    ...
    pub async fn set_draft(&self, conversation_id: &str, draft_text: &str) -> Result<()> {
        ...
        self.stores.conversation_dao.set_draft(conversation_id, draft_text, draft_time).await?;
    ...
    pub async fn get_pinned_conversations(&self) -> Result<Vec<LocalConversation>> {
        self.stores.conversation_dao.get_pinned().await
    }

    pub async fn count(&self) -> Result<usize> {
        self.stores.conversation_dao.count().await
    }

    pub async fn clear_all(&self) -> Result<()> {
        self.stores.conversation_dao.clear_all().await?;
        info!("会话数据已清空");
        Ok(())
    }
}
