...
impl MessageService {
    ...
    pub async fn mark_conversation_message_as_read(&self, conversation_id: String, _session_type: i32) -> Result<()> {
        ...
        // L49: GetConversation
        let conv = self.stores.conversation_dao.get_by_id(&conversation_id).await?;
        ...
        let peer_user_max_seq = self.stores.message_dao.get_peer_normal_msg_seq(&conversation_id, &user_id).await?;
        // L63: GetConversationNormalMsgSeq
        let mut max_seq = self.stores.message_dao.get_max_seq(&conversation_id).await?;
        if max_seq == 0 {
            max_seq = self.stores.conversation_dao.get_max_seq(&conversation_id).await?;
        }
        ...
        match conv.conversation_type {
            st if st == session_type::SINGLE_CHAT => {
                ...
                let unread_msgs = self.stores.message_dao.get_unread_messages(&conversation_id, &user_id).await?;
                ...
                if !msg_ids.is_empty() {
                    if let Err(e) = self.stores.message_dao
        ...
        // L98-100: UpdateColumnsConversation(unread_count=0)
        if let Err(e) = self.stores.conversation_dao.update_unread_count(&conversation_id, 0).await {
    ...
    async fn unread_change_trigger(&self, conversation_id: &str, latest_msg_is_read: bool) {
        ...
        // L167-168: ConChange
        let updated_conv = self.stores.conversation_dao.get_by_id(conversation_id).await;
    ...
    pub async fn mark_messages_as_read(
        ...
        if !seqs.is_empty() {
            self.stores.message_dao.mark_as_read_by_seqs(&conversation_id, &seqs, &user_id).await?;
        }
    ...
    pub async fn mark_all_conversation_as_read(&self) -> Result<()> {
        let conversations = self.stores.conversation_dao.get_all().await?;
        let user_id = self.user_id.get().await;

        for conv in &conversations {
            if conv.unread_count > 0 {
                // 为每个未读会话获取 maxSeq 并通知服务端
                let max_seq = self.stores.message_dao.get_max_seq(&conv.conversation_id).await.unwrap_or(0);
                if max_seq > 0 {
                    let _ = self.mark_conversation_as_read_server(&conv.conversation_id, max_seq, &[]).await;
                }
                // 标记本地消息已读 + 清零未读数
                self.stores.message_dao.mark_as_read_by_max_seq(&conv.conversation_id, max_seq, &user_id).await?;
                self.stores.conversation_dao.update_unread_count(&conv.conversation_id, 0).await?;
            }
        }
