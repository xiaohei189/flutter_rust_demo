...
mod tests {
    ...
    async fn test_existing_message_still_emits_conversation_changed() {
        ...
        let handler = MessageProcessor::new(repositories, UserId::new("user_a"), hub.clone(), crate::core::event::test_util::noop_message_listener());
    ...
    async fn test_no_trigger_conv_stored_but_no_conv_update() {
        ...
        let conversation_dao = repositories.conversation_repo.clone();
        let hub = crate::core::event::hub::EventHub::new();
        let handler = MessageProcessor::new(repositories, UserId::new(""), hub.clone(), crate::core::event::test_util::noop_message_listener());
    ...
    async fn test_typing_event_publishes_user_input_status() {
        let pool = create_pool_memory().await.unwrap();
        let hub = crate::core::event::hub::EventHub::new();
        let handler = MessageProcessor::new(make_test_repositories(pool), UserId::new("self_user"), hub.clone(), crate::core::event::test_util::noop_message_listener());
        let mut rx = hub.take_conv_rx().unwrap();
