//! 消息服务远程 API 抽象层
//!
//! 将 MessageService 对服务端的 HTTP 调用抽象为 trait，
//! 实现业务逻辑与传输层解耦，便于单元测试使用 mock。
...
/// 生产环境由 [`HttpMessageApi`] 实现，测试中可用 mock 替代。
#[async_trait]
...
mod tests {
    ...
    fn err_response() -> ResponseTemplate {
        ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "errCode": 1001, "errMsg": "message not found", "data": null
        }))
    }
    ...
    async fn test_revoke_on_server_business_error() {
        ...
        let result = api.revoke_on_server(&req).await;
    ...
    #[tokio::test]
    async fn test_delete_on_server_network_error() {
        // 不可达地址 → 网络错误
        let client = Arc::new(HttpApiClient::new(
        ...
        let api = HttpMessageApi::new(client);
        let result = api.delete_on_server("conv_1", &[1], "user_1").await;
        assert!(result.is_err());
    }

    // ====================================================================
    // mark_conversation_as_read_on_server
    // ====================================================================
    ...
    async fn test_mark_conv_read_success() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/msg/mark_conversation_as_read"))
            .respond_with(ok_response())
            .mount(&server).await;
    ...
    // ====================================================================
    // mark_messages_as_read_on_server
    // ====================================================================
    ...
    #[tokio::test]
    async fn test_mark_msgs_read_server_error() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/msg/mark_msgs_as_read"))
            .respond_with(err_response())
        ...
        assert!(api.mark_messages_as_read_on_server(&req).await.is_err());
    }
}
