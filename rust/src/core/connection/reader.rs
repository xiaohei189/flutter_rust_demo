...
use crate::core::connection::manager::PendingRequest;
...
use crate::core::event::events::connection::ConnectionListenerExt;
                    ...
                    msg = read.next() => {
                        match msg {
                                            ...
                                            match resp.req_identifier {
                                                ...
                                                ws_push_identifier::LOGOUT_MSG => {
                                                    ...
                                                    *is_manual_disconnect.write().await = true;
                                                    ...
                                                    listener.emit(crate::core::event::events::connection::ConnectionEvent::Logout);
                                                    cancel.cancel();
                                                    should_break = true;
                                                }
                                                ws_push_identifier::KICK_ONLINE_MSG => {
                                                    ...
                                                    *state.write().await = crate::core::connection::manager::ConnectionState::Kicked;
                                                    message_batcher.close().await;
                                                    listener.emit(crate::core::event::events::connection::ConnectionEvent::KickedOffline(resp.err_msg.to_string()));
                                                    cancel.cancel();
                                                    should_break = true;
                                                }
                            ...
                            Some(Ok(WsMessage::Close(_))) => {
                                ...
                                *state.write().await = crate::core::connection::manager::ConnectionState::Disconnected;
                                break;
                            }
                            Some(Err(e)) => {
