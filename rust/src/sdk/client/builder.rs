...
impl OpenIMClientBuilder {
    ...
    pub async fn build(self) -> Result<OpenIMClient> {
        ...
        let user = Arc::new(UserService::new(context.infra.http_client.clone(), listeners.clone()));
        let friend = Arc::new(FriendService::new(context.infra.http_client.clone(), context.repositories.clone(), context.user_id.clone(), listeners.clone()));
        let group = Arc::new(GroupService::new(context.infra.http_client.clone(), context.repositories.clone(), context.user_id.clone(), listeners.clone()));
        let conversation = Arc::new(ConversationService::new(context.repositories.clone(), listeners.clone()));
        let online_status = Arc::new(OnlineStatusService::new(context.infra.http_client.clone(), listeners.clone()));
