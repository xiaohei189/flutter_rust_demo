...
async fn test_create_group() {
    ...
    let result = sdk.create_group(&group_name, GroupType::Normal, std::slice::from_ref(&account.user_id)).await;
...
async fn test_join_and_quit_group() {
    ...
    let group = sdk1.create_group(&group_name, GroupType::Normal, std::slice::from_ref(&account1.user_id)).await.expect("创建群组失败");
    ...
    println!("用户1邀请用户2...");
    let invite_result = sdk1.invite_group_members(&group.group_id, std::slice::from_ref(&account2.user_id), None).await;
...
async fn test_group_member_management() {
    ...
    let group = sdk
        .create_group("MemberTestGroup", GroupType::Normal, std::slice::from_ref(&account.user_id))
        .await
        .expect("创建群组失败");
...
async fn test_group_info_update() {
    ...
    let group = sdk.create_group("InfoTestGroup", GroupType::Normal, std::slice::from_ref(&account.user_id)).await.expect("创建失败");
    ...
    println!("获取群信息...");
    let get_result = sdk.get_groups_info(std::slice::from_ref(&group.group_id)).await;
...
async fn test_group_dismiss_and_transfer() {
    ...
    // 邀请 account2
    sdk1.invite_group_members(&group.group_id, std::slice::from_ref(&account2.user_id), None).await.expect("邀请失败");
    tokio::time::sleep(Duration::from_secs(2)).await;
