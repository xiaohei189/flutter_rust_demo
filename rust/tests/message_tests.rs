...
async fn wait_for_event(events: &mut TestEvents, predicate: impl Fn(&TestEvent) -> bool, timeout_secs: u64) -> Option<TestEvent> {
...
async fn wait_for_new_messages(events: &mut TestEvents, count: usize, timeout_secs: u64) -> Vec<TestEvent> {
    let timeout = tokio::time::sleep(Duration::from_secs(timeout_secs));
    tokio::pin!(timeout);
...
async fn test_message_flow() {
    ...
    println!("[3/14] 高级文本消息...");
    let r = a_sdk.send_advanced_text_message("高级文本内容", vec![], target, st).await;
    ...
    println!("[8/14] 自定义消息...");
    let r = a_sdk
        .send_custom_message(r#"{"type":"gift","id":"rose_001"}"#, "送你一朵玫瑰花", r#"{"giftId":"rose_001"}"#, target, st)
        .await;
    ...
    println!("[10/14] @消息...");
    let r = a_sdk.send_at_text_message("@所有人 请注意", vec![user_b.user_id.clone()], target, st).await;
    ...
    let r = a_sdk
        .send_video_message_from_url("https://example.com/video.mp4", 10, "https://example.com/snapshot.jpg", target, st)
        .await;
    assert!(r.is_ok(), "发送URL视频失败: {:?}", r.err());
    a_offline_msg_count += 1;

    println!("[14/14] URL 文件...");
    let r = a_sdk.send_file_message_from_url("https://example.com/doc.pdf", "doc.pdf", 8192, target, st).await;
    ...
    for msg in &page2.messages {
        assert!(!page1_ids.contains(&msg.client_msg_id), "分页结果有重叠: {}", msg.client_msg_id);
    }
    println!("分页查询校验通过: 第一页 {} 条, 第二页 {} 条", page1.messages.len(), page2.messages.len());

    // 搜索本地消息
    println!("检查本地搜索...");
    use rust_lib_flutter_rust_demo::sdk::client::SearchMessagesReq;
    let search_result = b_sdk
    ...
    for msg in &search_result {
        assert!(msg.content.contains("UNIQUE_FLOW_42"), "搜索结果应包含关键词: {}", msg.content);
    }
    ...
    let conv_changed = wait_for_event(
        &mut b_events,
        |ev| {
            matches!(ev, TestEvent::Conversation(ConversationEvent::Changed(conversations))
    ...
    if !a_msgs.is_empty() {
        ...
        assert!(all_a_read, "A 发送的消息应全部已读，实际有 {} 条未读", a_msgs.iter().filter(|m| !m.is_read).count());
    }
    ...
    for i in 1..received_seqs.len() {
        assert_eq!(received_seqs[i], received_seqs[i - 1] + 1, "seq 不连续: {} → {}", received_seqs[i - 1], received_seqs[i]);
    }
    ...
    let convs = b_sdk.get_conversations().await.expect("获取会话失败");
    let conv = convs.iter().find(|c| c.conversation_id == conv_id).unwrap();
    assert_eq!(conv.unread_count, realtime_msg_count, "实时消息后未读数应为 {}，实际 {}", realtime_msg_count, conv.unread_count);
    ...
    let typing_ev = wait_for_event(&mut b_events, |ev| matches!(ev, TestEvent::Message(MessageEvent::NewMessage { .. })), 3).await;
    ...
    let convs = b_sdk.get_conversations().await.expect("获取会话失败");
    let conv = convs.iter().find(|c| c.conversation_id == conv_id).unwrap();
    assert_eq!(conv.unread_count, expected_unread, "全量已读前未读数应为 {}，实际 {}", expected_unread, conv.unread_count);
    ...
    let total_zero = wait_for_event(&mut b_events, |ev| matches!(ev, TestEvent::Conversation(ConversationEvent::TotalUnreadCountChanged(0))), 5).await;
...
async fn test_login_sync() {
    ...
    assert_eq!(conv.unread_count, offline_count, "未读数应为 {}，实际 {}", offline_count, conv.unread_count);
    println!("未读数校验通过: {}", conv.unread_count);
    ...
    assert!(history.is_ok(), "查询历史消息失败: {:?}", history.err());
    let history = history.unwrap();
    let offline_in_db = history.messages.iter().filter(|m| m.content.contains("离线同步测试")).count();
    assert_eq!(offline_in_db, offline_count as usize, "数据库应包含 {} 条离线消息，实际 {}", offline_count, offline_in_db);
...
async fn test_send_all_message_types() {
    ...
    // 10. 视频（URL）
    sender_sdk
        .send_video_message_from_url("https://example.com/video.mp4", 10, "https://example.com/snap.jpg", target, st)
        .await
        .unwrap();
    send_count += 1;
...
async fn test_history_query_reverse_and_by_seq() {
    ...
    // 筛选出本测试发送的消息
    let test_msgs: Vec<_> = history.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("REV_SEQ_MSG_")).collect();
    assert!(test_msgs.len() >= 10, "应至少有 10 条测试消息，实际 {}", test_msgs.len());
    ...
    let range_msgs: Vec<_> = all_history
        .messages
        .iter()
    ...
    assert!(range_msgs.len() >= 5, "seq 范围 {}~{} 内应有 >= 5 条消息，实际 {}", start_seq, end_seq, range_msgs.len());

    // 验证每条消息的 seq 都在范围内
    for msg in &range_msgs {
        assert!(msg.seq >= start_seq && msg.seq <= end_seq, "消息 seq={} 不在范围 {}~{} 内", msg.seq, start_seq, end_seq);
    }
    println!("Phase 5 通过: seq 范围 {}~{} 内有 {} 条消息", start_seq, end_seq, range_msgs.len());
    ...
    let single_msg = all_history.messages.iter().find(|m| m.seq == target_seq && m.send_id == sender.user_id);
    ...
    assert!(single_msg.content.contains("REV_SEQ_MSG_"), "消息内容应包含 REV_SEQ_MSG_: {}", single_msg.content);
...
async fn test_find_messages_by_ids() {
    ...
    for (i, sent_id) in sent_ids.iter().enumerate() {
        ...
        if let Some(msg) = found {
            assert!(msg.content.contains(&format!("FIND_MSG_{}", i + 1)), "消息内容不匹配: 期望 FIND_MSG_{}, 实际 {}", i + 1, msg.content);
            found_count += 1;
        }
    }
    ...
    let partial_history = receiver_sdk
        .get_history_messages(GetHistoryMessagesReq {
            conversation_id: conv_id.clone(),
            start_client_msg_id: String::new(),
            count: 50,
    ...
    let partial_found: Vec<_> = partial_history.messages.iter().filter(|m| partial_ids.contains(&m.client_msg_id)).collect();
    assert_eq!(partial_found.len(), 2, "应找到 2 条存在的消息，实际 {}", partial_found.len());
    ...
    let fake_found: Vec<_> = fake_history.messages.iter().filter(|m| fake_ids.contains(&m.client_msg_id)).collect();
    assert!(fake_found.is_empty(), "不应找到任何不存在的消息，实际找到 {}", fake_found.len());
    println!("Phase 5 通过: 不存在的 ID 返回空结果");

    // Phase 6: 空列表查询 → 验证返回空列表
    println!("\n========== Phase 6: 空列表查询 ==========");

    let empty_history = receiver_sdk
        .get_history_messages(GetHistoryMessagesReq {
            conversation_id: conv_id.clone(),
            start_client_msg_id: String::new(),
            count: 50,
...
async fn test_mark_specific_messages_as_read() {
    ...
    assert!(conv.is_some(), "未找到会话 {}", conv_id);
    let conv = conv.unwrap();
    assert_eq!(conv.unread_count, 5, "未读数应为 5，实际 {}", conv.unread_count);
    ...
    let test_msgs: Vec<_> = history.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("MARK_READ_")).collect();
    ...
    let history_after = receiver_sdk
        .get_history_messages(GetHistoryMessagesReq {
            conversation_id: conv_id.clone(),
            start_client_msg_id: String::new(),
            count: 50,
    ...
    let test_msgs_after: Vec<_> = history_after.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("MARK_READ_")).collect();
    let mut sorted_after: Vec<_> = test_msgs_after.into_iter().collect();
    sorted_after.sort_by_key(|m| m.seq);

    // 前 3 条应已读
    for msg in &sorted_after[0..3] {
        assert!(msg.is_read, "前 3 条应已读: seq={}, content={}, is_read={}", msg.seq, msg.content, msg.is_read);
    }
    println!("  前 3 条 is_read == true ✓");

    // 后 2 条应未读
    for msg in &sorted_after[3..5] {
        assert!(!msg.is_read, "后 2 条应未读: seq={}, content={}, is_read={}", msg.seq, msg.content, msg.is_read);
    }
...
async fn test_delete_message_local_only() {
    ...
    let test_msgs: Vec<_> = history.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("LOCAL_DEL_")).collect();
    assert_eq!(test_msgs.len(), 3, "应有 3 条测试消息，实际 {}", test_msgs.len());
    println!("Phase 3 通过: 3 条消息都在本地");

    // Phase 4: B 调用 delete_messages 删除第 2 条
    println!("\n========== Phase 4: B 删除第 2 条消息 ==========");

    let del_result = receiver_sdk
        .delete_messages(DeleteMessagesReq {
            conversation_id: conv_id.clone(),
    ...
    let remaining: Vec<_> = history_after.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("LOCAL_DEL_")).collect();

    // 第 2 条应不在（已被删除）
    let second_msg_found = remaining.iter().any(|m| m.client_msg_id == sent_ids[1]);
    assert!(!second_msg_found, "第 2 条消息应已被删除，但仍存在");
...
async fn test_clear_conversation_and_delete_all_msg() {
    ...
    let test_msgs_before: Vec<_> = history.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("CLEAR_DEL_")).collect();
    assert!(test_msgs_before.len() >= 5, "应有 >= 5 条测试消息，实际 {}", test_msgs_before.len());
    ...
    let test_msgs_after: Vec<_> = history_after.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("CLEAR_DEL_")).collect();
    assert!(test_msgs_after.is_empty(), "测试消息应已清空，实际仍有 {} 条", test_msgs_after.len());
...
async fn test_delete_conversation_and_delete_all_msg() {
    ...
    assert!(conv_after.is_none(), "会话应已不存在，但仍存在: {:?}", conv_after);
    ...
    let remaining: Vec<_> = history.messages.iter().filter(|m| m.send_id == sender.user_id && m.content.contains("CONV_DEL_")).collect();
    assert!(remaining.is_empty(), "消息应已清空，实际仍有 {} 条", remaining.len());
    ...
    assert!(new_conv.is_some(), "新会话应被创建 {}", conv_id);
    println!("Phase 6 通过: 新会话已创建");

    // 验证新消息存在
    let new_history = receiver_sdk2
        .get_history_messages(GetHistoryMessagesReq {
            conversation_id: conv_id.clone(),
            start_client_msg_id: String::new(),
            count: 10,
    ...
    let new_msg_found = new_history.messages.iter().any(|m| m.content.contains("CONV_DEL_NEW"));
...
async fn test_quote_message_flow() {
    ...
    println!("  原始消息: client_msg_id={}, seq={}", original_msg_data.client_msg_id, original_msg_data.seq);
    ...
    let quote_result = sender_sdk.send_quote_message("这是引用回复", quote_msg, target, st).await;
    assert!(quote_result.is_ok(), "发送引用消息失败: {:?}", quote_result.err());
    let quote_msg_data = quote_result.unwrap();
    println!("  引用消息: client_msg_id={}, content_type={}", quote_msg_data.client_msg_id, quote_msg_data.content_type);
    ...
    // 查找引用消息
    let quote_found = history.messages.iter().find(|m| m.client_msg_id == quote_msg_data.client_msg_id);
    assert!(quote_found.is_some(), "引用消息应存在于历史中: client_msg_id={}", quote_msg_data.client_msg_id);
    let quote_found = quote_found.unwrap();
    println!("  引用消息找到: content_type={}, content={}", quote_found.content_type, quote_found.content);

    // 验证原始消息也存在
    let original_found = history.messages.iter().find(|m| m.client_msg_id == original_msg_data.client_msg_id);
    assert!(original_found.is_some(), "原始消息应存在于历史中: client_msg_id={}", original_msg_data.client_msg_id);
    ...
    .await;
    assert!(ev.is_some(), "B 未收到引用消息的 NewMessage 事件: client_msg_id={}", quote_msg_data.client_msg_id);
...
async fn test_message_local_ex() {
    ...
    let find_msg = |client_msg_id: &str| -> Option<&LocalChatLog> { all_msgs.iter().find(|m| m.client_msg_id == client_msg_id) };
...
async fn test_group_message_flow() {
    ...
    println!("\n========== Phase 2: A 创建群组 ==========");
    let group = sdk_a
        .create_group("测试群", GroupType::Normal, &[account_b.user_id.clone(), account_c.user_id.clone()])
        .await
        .expect("创建群组失败");
    ...
    assert_eq!(conv_b.unread_count, 5, "B 的群未读数应为 5，实际 {}", conv_b.unread_count);
    ...
    assert_eq!(conv_c.unread_count, 5, "C 的群未读数应为 5，实际 {}", conv_c.unread_count);
    ...
    let history = sdk_b
        ...
        .await
        .unwrap();
    let group_msgs: Vec<_> = history.messages.iter().filter(|m| m.content.contains("GROUP_MSG_")).collect();
    ...
    assert_eq!(conv_b.unread_count, 0, "B 标记已读后未读数应为 0，实际 {}", conv_b.unread_count);
    ...
    assert_eq!(conv_c.unread_count, 0, "C 标记已读后未读数应为 0，实际 {}", conv_c.unread_count);
...
async fn test_online_only_message() {
    ...
    assert_eq!(received_online_only.len(), 3, "B 应实时收到 3 条 online_only 消息，实际 {}", received_online_only.len());
    ...
    let conv = b_sdk.get_conversation(&conv_id).await.unwrap().unwrap();
    assert_eq!(conv.unread_count, 0, "online_only 消息不应增加未读数，期望 0，实际 {}", conv.unread_count);
    ...
    let online_in_history: Vec<_> = history.messages.iter().filter(|m| online_only_ids.contains(&m.client_msg_id)).collect();
    assert!(online_in_history.is_empty(), "online_only 消息不应在历史中，但仍找到 {} 条", online_in_history.len());
    println!("Phase 6 通过: online_only 消息不在历史中");

    // 验证普通基线消息仍在历史中
    let baseline_in_history: Vec<_> = history.messages.iter().filter(|m| m.content.contains("普通基线消息")).collect();
    assert_eq!(baseline_in_history.len(), 2, "基线消息应在历史中，期望 2 条，实际 {}", baseline_in_history.len());
    println!("  基线普通消息在历史中 ✓");
    ...
    tokio::time::sleep(Duration::from_secs(1)).await;
    let conv = b_sdk.get_conversation(&conv_id).await.unwrap().unwrap();
    assert_eq!(conv.unread_count, 1, "混合发送后未读数应为 1（仅普通消息），实际 {}", conv.unread_count);
...
async fn test_msg_edit_notification() {
    ...
    use rust_lib_flutter_rust_demo::sdk::client::{GetHistoryMessagesReq, SearchMessagesReq};
    ...
    assert_eq!(found.local_ex, "edited_v2", "local_ex 应为 edited_v2，实际: {}", found.local_ex);
    ...
    let a_msg = a_history.messages.iter().find(|m| m.client_msg_id == msg_data.client_msg_id);
    ...
    assert_eq!(a_text, "原始消息内容_v1", "A 侧消息内容应为 '原始消息内容_v1'，实际: {}", a_text);
    ...
    let b_msg = b_history.messages.iter().find(|m| m.client_msg_id == msg_data.client_msg_id);
    ...
    assert_eq!(b_text, "原始消息内容_v1", "B 侧消息内容应为 '原始消息内容_v1'，实际: {}", b_text);
    ...
    let edit_test_msgs: Vec<_> = all_history.messages.iter().filter(|m| m.content.contains("EDIT_TEST_MSG_")).collect();
    assert_eq!(edit_test_msgs.len(), 3, "应有 3 条 EDIT_TEST_MSG，实际 {}", edit_test_msgs.len());
...
async fn test_concurrent_send_stress() {
    ...
    assert_eq!(received_count, 20, "B 应收到全部 20 条消息，实际 {}", received_count);
    ...
    assert!(
        ...
        seq_range,
        &received_seqs[..3],
        &received_seqs[received_seqs.len() - 3..]
    );
    ...
    let a_custom_fut = a_sdk_arc.send_custom_message(r#"{"type":"stress"}"#, "并发自定义", r#"{}"#, target_b, st);
    ...
    assert_eq!(mixed_received, 3, "B 应收到 3 条混合类型消息，实际 {}", mixed_received);
    ...
    let stress_msgs: Vec<_> = final_history.messages.iter().filter(|m| m.content.contains("STRESS_SEQ_")).collect();
    let bidir_msgs: Vec<_> = final_history.messages.iter().filter(|m| m.content.contains("BIDIR_")).collect();
    let mixed_msgs: Vec<_> = final_history.messages.iter().filter(|m| m.content.contains("并发")).collect();
    ...
    assert_eq!(conv.unread_count, 0, "最终 A 未读数应为 0，实际 {}", conv.unread_count);
...
async fn test_send_sound_message_flow() {
    ...
    let sound_result = sender_sdk.send_sound_message(wav_path.to_str().unwrap(), target, st, 1).await;
    ...
    let mut b_events = subscribe_all(&receiver_sdk);
    let sound_result2 = sender_sdk.send_sound_message(wav_path.to_str().unwrap(), target, st, 2).await;
...
async fn test_send_video_message_flow() {
    ...
    let video_result = sender_sdk.send_video_message(mp4_path.to_str().unwrap(), snapshot_path.to_str().unwrap(), target, st, 10).await;
    ...
    let mut b_events = subscribe_all(&receiver_sdk);
    let video_result2 = sender_sdk.send_video_message(mp4_path.to_str().unwrap(), snapshot_path.to_str().unwrap(), target, st, 5).await;
...
async fn test_edit_message_real() {
    ...
    let edit_not_exist = sender_sdk.edit_message(&conv_id, "non_existent_msg_id", "新内容", 101).await;
...
async fn test_delete_all_msg_local_and_svr() {
    ...
    let test_msgs: Vec<_> = history.messages.iter().filter(|m| m.content.contains("DELALL_MSG_")).collect();
    ...
    let remaining: Vec<_> = history_after.messages.iter().filter(|m| m.content.contains("DELALL_MSG_")).collect();
...
async fn test_delete_all_msg_local_only() {
    ...
    let test_msgs_before: Vec<_> = history.messages.iter().filter(|m| m.content.contains("DELLOCAL_MSG_")).collect();
    ...
    let remaining: Vec<_> = history_after.messages.iter().filter(|m| m.content.contains("DELLOCAL_MSG_")).collect();
    assert!(remaining.is_empty(), "本地软删除后测试消息应不可见，实际有 {} 条", remaining.len());
