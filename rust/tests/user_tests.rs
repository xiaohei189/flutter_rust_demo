...
async fn test_user_state_via_sdk() {
    ...
    println!("6. 获取用户信息...");
    let users_result = sdk.get_users_info(std::slice::from_ref(&user_id)).await;
...
async fn test_get_user_online_status() {
    ...
    let status = sdk.get_user_status(std::slice::from_ref(&user1.user_id)).await;
...
async fn test_update_user_profile() {
    ...
    println!("验证用户资料...");
    let users_result = sdk.get_users_info(std::slice::from_ref(&user1.user_id)).await;
...
async fn test_subscribe_unsubscribe_user_status() {
    ...
    let _ = sdk2.get_user_status(std::slice::from_ref(&user2.user_id)).await;
