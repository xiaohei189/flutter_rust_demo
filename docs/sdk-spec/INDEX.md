...
> ⚠️ **以代码为准**：本文档集是**规格参考**（对齐 Go SDK 的协议 / 数据流 / 常量 / 模型），其中「实施状态 / 缺失功能 / ❌ 未实现」等**状态标注为早期基线，与当前代码存在大量出入**（多数功能后续已实现但未回写文档）。
> 功能是否实现的**权威依据**：实际代码（`rust/src/`、`rust/tests/`）+ **[docs/testing-gap.md](../testing-gap.md)**。
> 常量、数据模型、HTTP 路由、协议流程等**规格性内容**仍可直接参考。
