...
class _FriendSetupScreenState extends ConsumerState<FriendSetupScreen> {
  ...
  Future<void> _updateRemark(String remark) async {
    ...
    try {
      await FriendService.instance.updateFriends(
        client,
        friendUserIds: [widget.userId],
        remark: remark,
      );
      if (mounted) {
  ...
  Future<void> _toggleBlacklist(bool value) async {
    ...
    try {
      if (value) {
        ...
        await FriendService.instance.addBlack(
          client,
        ...
        await FriendService.instance.removeBlack(
          client,
          userId: widget.userId,
        );
      ...
      appLog.e('[FriendSetupScreen] 设置黑名单失败: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('操作失败: $e'),
  ...
  Future<void> _confirmDeleteFriend() async {
    ...
    try {
      ...
      appLog.e('[FriendSetupScreen] 删除好友失败: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('删除失败: $e'),
