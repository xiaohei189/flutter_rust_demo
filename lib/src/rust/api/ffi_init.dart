...
Future<void> setLogDirectory({required String path}) =>
    RustLib.instance.api.crateApiFfiInitSetLogDirectory(path: path);

/// 初始化 Rust 日志系统（兼容旧接口，内部委托给 init_otel_subscriber）
Future<void> initLogger({required String logLevel}) =>
    RustLib.instance.api.crateApiFfiInitInitLogger(logLevel: logLevel);

/// 初始化日志系统（完整配置）
Future<void> initLoggerV2({required LogConfig config}) =>
    RustLib.instance.api.crateApiFfiInitInitLoggerV2(config: config);
...
Future<void> setLogSpanEvents({required bool enabled}) =>
    RustLib.instance.api.crateApiFfiInitSetLogSpanEvents(enabled: enabled);
